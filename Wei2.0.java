...
{  
    ...
    //按压的时间系数，可根据具体情况适当调节  
    private final double        pressTimeCoefficient    = 1.43;  
    ...
    {  
        ...
        //计算按压时间，最小300毫秒  
        int pressTime = (int) Math.max(distance * pressTimeCoefficient, 300);  
        System.out.println("pressTime: " + pressTime);  
        //执行按压操作  
        String command = String.format("adb shell input swipe %s %s %s %s %s",
                 swipeX + rand.nextInt(50), swipeY + rand.nextInt(50), swipeX + rand.nextInt(50), swipeY + rand.nextInt(50),  
                pressTime);  
